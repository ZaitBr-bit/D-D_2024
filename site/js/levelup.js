...
import { clonarFonte } from './fontes.js';
...
export async function subirDeNivel(personagem, opcoes = {}) {
  ...
  if (exigeGrimorioMago) {
    ...
    magiasGrimorioSelecionadas = selecionadas.map(nome => {
      const magia = magiasPorNome.get(nome);
      const fonte = clonarFonte(magia.fonte);
      return { nome: magia.nome, circulo: magia.circulo, ...(fonte ? { fonte } : {}) };
    });
  }
  ...
  if (ganhaAumentoAtributo && opcoes.talento) {
    ...
    if (opcoes.talento_tipo_escolha === 'tocado_fadas' || opcoes.talento_tipo_escolha === 'tocado_sombras') {
      ...
      const indiceTF = await getIndiceMagias();
      const magiasPorNomeTF = new Map((indiceTF?.magias || []).map(magia => [magia.nome, magia]));
      // Magia escolhida (1º círculo)
      if (nomeMagia && !personagem.magias_preparadas.find(m => m.nome === nomeMagia)) {
        const fonte = clonarFonte(magiasPorNomeTF.get(nomeMagia)?.fonte);
        personagem.magias_preparadas.push({ nome: nomeMagia, circulo: 1, origem, gratis_usado: false, ...(fonte ? { fonte } : {}) });
      }
      ...
      if (!personagem.magias_preparadas.find(m => m.nome === nomeParceiro)) {
        const fonteParceiro = clonarFonte(magiasPorNomeTF.get(nomeParceiro)?.fonte);
        personagem.magias_preparadas.push({ nome: nomeParceiro, circulo: 2, origem, gratis_usado: false, ...(fonteParceiro ? { fonte: fonteParceiro } : {}) });
      }
    }
    ...
    if (opcoes.talento_tipo_escolha === 'conjurador_ritualista') {
      if (!personagem.magias_preparadas) personagem.magias_preparadas = [];
      const indiceCR = await getIndiceMagias();
      const magiasPorNomeCR = new Map((indiceCR?.magias || []).map(magia => [magia.nome, magia]));
      for (const nomeMagia of (opcoes.escolhas_talento_levelup || [])) {
        if (!personagem.magias_preparadas.find(m => m.nome === nomeMagia)) {
          const fonte = clonarFonte(magiasPorNomeCR.get(nomeMagia)?.fonte);
          personagem.magias_preparadas.push({ nome: nomeMagia, circulo: 1, origem: 'conjurador_ritualista', ...(fonte ? { fonte } : {}) });
        }
      }
    }
    ...
    if (opcoes.talento_tipo_escolha === 'iniciado_em_magia' && opcoes.iniciado_em_magia) {
      ...
      const indiceIM = await getIndiceMagias();
      const magiasPorNomeIM = new Map((indiceIM?.magias || []).map(magia => [magia.nome, magia]));
      // Adicionar truques às magias conhecidas
      if (!personagem.magias_conhecidas) personagem.magias_conhecidas = [];
      for (const nome of (im.truques || [])) {
        if (!personagem.magias_conhecidas.find(m => m.nome === nome)) {
          const fonte = clonarFonte(magiasPorNomeIM.get(nome)?.fonte);
          personagem.magias_conhecidas.push({ nome, circulo: 0, origem: 'iniciado_em_magia', ...(fonte ? { fonte } : {}) });
        }
      }
      ...
      if (im.magia) {
        ...
        if (!personagem.magias_preparadas.find(m => m.nome === im.magia)) {
          const fonteMagia = clonarFonte(magiasPorNomeIM.get(im.magia)?.fonte);
          personagem.magias_preparadas.push({ nome: im.magia, circulo: 1, origem: 'iniciado_em_magia', gratis_usado: false, ...(fonteMagia ? { fonte: fonteMagia } : {}) });
        }
      }
    }
  }
