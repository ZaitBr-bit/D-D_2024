  ...
  document.getElementById('btn-salvar-im-edit')?.addEventListener('click', () => {
    ...
    for (const nome of estado.truques) {
      if (!char.magias_conhecidas.find(m => m.nome === nome && m.circulo === 0)) {
        const fonte = clonarFonte(truquesDisp.find(m => m.nome === nome)?.fonte);
        char.magias_conhecidas.push({ nome, circulo: 0, origem: 'iniciado_em_magia', ...(fonte ? { fonte } : {}) });
      }
    }
    ...
    if (oldMagia !== estado.magia) {
      ...
      if (!char.magias_preparadas.find(m => m.nome === estado.magia)) {
        const fonte = clonarFonte(c1Disp.find(m => m.nome === estado.magia)?.fonte);
        char.magias_preparadas.push({ nome: estado.magia, circulo: 1, origem: 'iniciado_em_magia', gratis_usado: false, ...(fonte ? { fonte } : {}) });
      }
    }
...
async function mostrarTrocaMagias(callbackPosTroca = null) {
  ...
  document.getElementById('btn-confirmar-troca')?.addEventListener('click', () => {
    const novasPreparadas = [...selecionadasSet].map(nome => criarEntradaMagiaFicha(nome, circuloMap[nome] || 1));
