// ============================================================
// ORÁCULO LEGADO — cópia CONGELADA do que era `site/js/pages/sheet.js` até a
// Task 33 (cutover público da ficha).
//
// ## O que este arquivo é (e o que NÃO é)
//
// Este É o `site/js/pages/sheet.js` original — o monólito de ~18k linhas —,
// apenas movido de lugar pela Task 33, exatamente como a Task 22b fez com
// `site/js/db.js` -> `tests/helpers/legacy-db-source.js`. Ele NÃO é mais
// código de produção: nenhum módulo de `site/js/**` o importa, ele não é
// publicado no deploy (o workflow copia `site/` e `dados/`, nunca `tests/`), e
// a ficha viva passou a ser o composition root fino de `site/js/pages/sheet.js`
// sobre `features/sheet/**`.
//
// ## Por que ele continua existindo
//
// Porque ele é o ORÁCULO de contratos que medem a migração contra o CÓDIGO
// REAL do baseline, e não contra uma transcrição dele:
//
//   - `tests/contract/spell-parity.test.js` extrai daqui as chaves de
//     `MAGIAS_EFEITO` e os nomes de `OPCOES_METAMAGIA` para exigir que toda
//     magia que o baseline automatizava exista no catálogo (e que o gap de
//     metamagia continue registrado como asserção, não como comentário).
//
// Apagá-lo faria essas asserções compararem a migração consigo mesma e ficarem
// verdes sem provar nada. Ele é imutável: qualquer necessidade de mudar o
// comportamento pertence a `site/js/features/sheet/**`, nunca aqui.
//
// NÃO EXECUTE este módulo. Os imports relativos a `../utils.js` etc. apontam
// para caminhos que não existem a partir de `tests/helpers/`; ele é lido como
// TEXTO pelos contratos acima, e é assim que deve continuar.
// ============================================================
// ============================================================
...
// Task 18 (estrangulamento): a regra "efeito de concentração cai quando a
// concentração muda" passa a ter UMA fonte — `domain/spells/concentration.js`.
// A ficha ainda opera sobre o registro plano legado (`char.efeitos_magicos`),
// então delega só a primitiva de filtragem; o comando canônico completo
// (`set-concentration`/`end-concentration` no dispatcher) entra quando
// sheet.js migrar para o personagem canônico.
import { filtrarEfeitosSemConcentracao } from '../domain/spells/index.js';
...
function registrarConcentracaoMagiaPersonalizada(magia, circulo) {
  ...
  char.efeitos_magicos = filtrarEfeitosSemConcentracao(char.efeitos_magicos);
